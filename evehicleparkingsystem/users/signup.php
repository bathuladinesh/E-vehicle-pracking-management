...
if (isset($_POST['submit'])) {
    ...
    $contno = sanitizeInput($_POST['mobilenumber']);
    ...
    if (!isValidPhoneNumber($contno)) {
        echo '<script>alert("Invalid phone number. Please provide a valid number.");</script>';
        exit;
    }

    $stmt = $con->prepare("SELECT Email FROM tblregusers WHERE Email = ? OR MobileNumber = ?");
    if ($stmt === false) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt = $con->prepare("INSERT INTO tblregusers (FirstName, LastName, MobileNumber, Email, Password) VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . $con->error);
        }
        $stmt->bind_param("sssss", $fname, $lname, $contno, $email, $password);
        if ($stmt->execute()) {
